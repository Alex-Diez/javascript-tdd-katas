...
export default class LinkedDeque extends React.Component {
    ...
    render() {
        ...
        function prettyPrintedDeque() {
            const items = self.state.items;
            const prettyPrintedItem = [];
            const length = items.length;
            if (length !== 0) {
                prettyPrintedItem.push((<span>&lt;-</span>));
                for (let i = 0; i < length - 1; i++) {
                    prettyPrintedItem.push(items[i]);
                    prettyPrintedItem.push((<span>&lt;=&gt;</span>));
                }
                prettyPrintedItem.push(items[length - 1]);
                prettyPrintedItem.push((<span>-&gt;</span>));
            }
            return prettyPrintedItem;
        }
