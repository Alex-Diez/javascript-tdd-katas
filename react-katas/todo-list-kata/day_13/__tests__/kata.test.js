...
describe('todo list', () => {
    ...
    test('canceled task has disabled complete and cancel buttons', () => {
        createNumberOfTasks(3);

        cancelTaskAt(1);

        const task = taskList.find('.canceled');
        const cancelTaskButton = task.find('.cancel');
        const completeTaskButton = task.find('.complete');

        expect(cancelTaskButton.props().disabled).toBe(true);
        expect(completeTaskButton.props().disabled).toBe(true);
    });

    test('completed task has disabled complete and cancel buttons', () => {
        createNumberOfTasks(3);
